...
class Scenario extends AbstractHelper
{
     ...
     * @return array
     */
    public function setAllFrag($query)
    {
        ...
        if ($rs) {
            ...
            if (count($exist) == 0) {
                ...
                return ['media' => $media,'item' => $exist[0]];
            }
        } else {
            return ['error' => "droits insuffisants",'message' => "Vous n'avez pas le droit de créer des fragments."];
        }
    }
     ...
     * @return array
     */
    public function setAleaFrag($query)
    {
        //récupère un média
        $query = [];
     ...
     * @return array
     */
    public function showChoix($post)
    {
     ...
     * @return string
     */
    public function formatWebVTTtimestamp($ts)
    {
        $arr = explode(':', $ts);
        if (count($arr) == 2) {
     ...
     * @return array
     */
    public function addLayers($idScenario, $post)
    {
        $rs = $this->acl->userIsAllowed(null, 'create');
        if ($rs) {
            $rs = [];
     ...
     * @return array
     */
    public function getRtSuggestion($label, $urlApi)
    {
        $props = [];
     ...
     * @return array
     */
    public function scenarioTxtToObj($idItem)
    {
        $rs = $this->acl->userIsAllowed(null, 'create');
        if ($rs) {
     ...
     * @return array
     */
    public function genereScenario($query, $post)
    {
     ...
     * @return array
     */
    public function deleteLayer($post)
    {
     ...
     * @return array
     */
    public function deleteTrack($post)
    {
        $rs = $this->acl->userIsAllowed(null, 'delete');
        if ($rs) {
     ...
     * @return o:item
     */
    public function createTrack($post)
    {
        $rs = $this->acl->userIsAllowed(null, 'create');
        if ($rs) {
     ...
     * @return array
     */
    public function getTimelinerTrack($post, $i)
    {
        if (!$i) {
            $i = $this->api->read('items', $post['oItem']->id())->getContent();
        }
     ...
     * @return o:item
     */
    public function saveTrack($params, $rtLabel)
    {
        //enregistre une indexation dans la base
     ...
     * @return array
     */
    public function getScenarioTracks($id)
    {
        $param = [];
     ...
     * @return array
     */
    public function genScenario($query, $post = false)
    {
    ...
    public function createTimelinerEntry($idGroup, $category, $i)
    {
        ...
        if ($te == $ts) {
            $te += 5;
        }//ajoute des secondes pour voir la track
     ...
     * @return array
     */
    public function addRessourceInfos($p, $i, &$l)
    {
     ...
     */
    protected function setValeur($val, $oP, &$oItem): void
    {
        if (is_string($val)) {
            $val = [$val];
        }
     ...
     */
    public function groupByTitre($data)
    {
        $result = [];
        foreach ($data as $item) {
            ...
            if (!array_key_exists($val, $result)) {
                $result[$val] = ['items' => [],'id' => $id,'color' => $color,'desc' => $desc];
            }
     ...
     * @param {Array} $data Array that stores multiple item.
     */
    public function groupByClassTitre($data)
    {
        $result = [];
        foreach ($data as $item) {
            //construction de la clef
     ...
     * @param {Array} $data Array that stores multiple item.
     */
    public function groupByCategoryCreator($data)
    {
        $result = [];
        foreach ($data as $item) {
            // Construction de la clef
     ...
     * @param {Array} $data Array that stores multiple item.
     */
    public function groupBySourceClassTarget($data)
    {
        $result = [];
        foreach ($data as $item) {
            // Construction de la clef
     ...
     * @param {Array} $data Array that stores multiple item.
     */
    public function groupBySourceClassBody($data)
    {
        $result = [];
        foreach ($data as $item) {
            // Construction de la clef
     ...
     * @param {Array} $data Array that stores multiple item.
     */
    public function groupByScopeSourceClass($data)
    {
        $result = [];
        foreach ($data as $item) {
            // Construction de la clef
            ...
            $desc = '';
            if (!array_key_exists($val, $result)) {
                $result[$val] = ['items' => [],'id' => $id,'source' => $source,'class' => $rc,'color' => $color,'desc' => $desc];
            }
            $result[$val]['items'][] = $item;
        }
        ksort($result);
