...
class DiagrammeViewHelper extends AbstractHelper
{
    ...
    function deleteGeo($params){
        $rs = $this->api->delete('items', $params['id']);
        return $rs;
    }

    function createDiagramme($params){
    ...
    function createNode($params){
        ...
        if(isset($params['cssStyle']))$data['oa:styleClass'][]=['@value'=>$params['cssStyle'],'property_id'=>$this->getProp('oa:styleClass')->id(),'type'=>'literal'];
        ...
        //création de l'item
        $oItem = $this->api->create('items', $data, [], ['continueOnError' => true])->getContent();
        if(isset($params['id']))$this->nodes[$params['id']]=$oItem->id();  
        ...
        $this->api->update('items', $params['idDiagram'], $data, []
    ...
    function getDiagramme($p,$oItem=false){
        if(!$oItem) $oItem =  $this->api->read('items',$p['query']['id'])->getContent();
    ...
    function getGeoInfo($oItem, $idDiagram){
    ...
    protected function ajouteLinks($data, $oItemCarte, $arrEntities)
    {
        ...
        foreach ($data['ivml:links']['ivml:link'] as $k => $d) {
            ...
            //création le concept
            if($d['label']) $this->getTag($d['label'],$oItem);
